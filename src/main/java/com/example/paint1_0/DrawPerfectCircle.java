...
public class DrawPerfectCircle extends DrawShape {
    private boolean esRelleno = false;
    protected Double x2 = 0D;
    protected Double y2 = 0D;

    public DrawPerfectCircle(GraphicsContext graphicsContext) {
    ...
    public void draw(ColorPicker colorPicker, TextField bSize) {
        ...
        graphicsContext.getCanvas().setOnMouseReleased(mouseEvent -> {
            ...
            if (esRelleno) {
                ...
                graphicsContext.fillOval(startX, startY, width, width);

                esRelleno = false;
            } else {
                graphicsContext.strokeOval(startX, startY, width, width);
            }
